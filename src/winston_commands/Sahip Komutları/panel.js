...
module.exports = {
    ...
    execute: async (client, message, args, winston_embed) => {
        ...
        const winston_buttons_two = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setLabel(`Kurulum Hakkında Bilmeniz Gerekenler`)
                    .setEmoji(`ℹ️`)
                    .setStyle(ButtonStyle.Link)
                    .setURL("https://github.com/winston-js/advanced-v14-register")
            )
        ...
        winston_embed.setDescription(`
**Kayıt Yetkilileri \`ID: 1\`**
${staffData.length > 0 ? staffData.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Erkek Rolleri \`ID: 2\`**
${manRoles.length > 0 ? manRoles.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Kadın Rolleri \`ID: 3\`**
${womanRoles.length > 0 ? womanRoles.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Kayıtsız Rolleri \`ID: 4\`**
${unregisterRoles.length > 0 ? unregisterRoles.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Jail Rolleri \`ID: 5\`**
${jailRoles.length > 0 ? jailRoles.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Family/Taglı Rolleri \`ID: 6\`**
${familyRoles.length > 0 ? familyRoles.map((winston) => `<@&${winston}>`).join(",") : "Bulunmamakta"}
**Taglar \`ID: 7\`**
${tagData.length > 0 ? tagData.map((winston) => `${winston}`).join(",") : "Bulunmamakta"}
...
`).setThumbnail(message.guild.iconURL({ forceStatic: true }))
        ...
        if (!secim) return message.reply({ components: [winston_buttons, winston_buttons_two], embeds: [winston_embed] })
        ...
        if (secim == "1") {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 1 @rol**` })
            if (staffData.some(winston => winston.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 1 @rol**` })
            if (manRoles.some(winston => winston.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 3 @rol**` })
            if (womanRoles.some(winston => winston.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 4 @rol**` })
            if (unregisterRoles.some(winston => winston.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 5 @rol**` })
            if (jailRoles.some(winston => winston.includes(roles.id))) {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 6 @rol**` })
            if (familyRoles.some(winston => winston.includes(roles.id))) {
            ...
            if (!tag) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 7 <tag>**` })
            if (tagData.some(winston => winston.includes(tag))) {
            ...
            if (!channel) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 9 #chat-kanal**` })
            ...
            if (!channel) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 10 #ses-kanal**` })
            ...
            if (!channel) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup 11 #log-kanal**` })
            db.set("five-channel-log", channel.id)
            message.reply({ content: `> **✅ Başarılı!**\n> **${channel} (\`${channel.name}\`) Başarıyla Eklendi!**` })

        }else{ return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${winston_config.prefix}setup <ID> @rol/#kanal/tag**` }) }
...
client.on("interactionCreate", async (winston) => {
    if (!winston.isButton()) return;
    let fiveValue = winston.customId;
let noPermMessage = `> **❌ Bu İşlem İçin \` Yönetici \` Yetkisine Sahip Olmalısın!**`;
        ...
        const winston_buttons = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    ...
                    .setEmoji(`ℹ️`)
                    .setStyle(ButtonStyle.Link)
                    .setURL("https://github.com/winston-js")
            )
    ...
    if (fiveValue == "welcome_image") {
        if(!winston.member.permissions.has(PermissionFlagsBits.Administrator))return winston.reply({content:noPermMessage,ephemeral:true})
        if (!db.has("five-welcome-image")) {
            db.set("five-welcome-image", true);
            winston.reply({ content: `> **✅ Canvaslı / Resimli Hoşgeldin Başarıyla Açıldı!**`, ephemeral: true ,components:[winston_buttons] })
        } else {
            db.delete("five-welcome-image");
            winston.reply({ content: `> **✅ Canvaslı / Resimli Hoşgeldin Başarıyla Kapatıldı!**`, ephemeral: true ,components:[winston_buttons] })
        }
    } else if (fiveValue == "welcome_mentions") {
        if(!winston.member.permissions.has(PermissionFlagsBits.Administrator))return winston.reply({content:noPermMessage,ephemeral:true})
        if (!db.has("five-welcome-mentions")) {
            db.set("five-welcome-mentions", true);
            winston.reply({ content: `> **✅ Hoşgeldin Rol Etiketi Başarıyla Açıldı!**`, ephemeral: true,components:[winston_buttons]  })
        } else {
            db.delete("five-welcome-mentions");
            winston.reply({ content: `> **✅ Hoşgeldin Rol Etiketi Başarıyla Kapatıldı!**`, ephemeral: true,components:[winston_buttons] })
        }
    } else if (fiveValue == "tag_mode") {
        if(!winston.member.permissions.has(PermissionFlagsBits.Administrator))return winston.reply({content:noPermMessage,ephemeral:true})
        if (!db.has("five-welcome-tagmode")) {
            db.set("five-welcome-tagmode", true);
            winston.reply({ content: `> **✅ Taglı Alım Başarıyla Açıldı!**`, ephemeral: true,components:[winston_buttons] })
        } else {
            db.delete("five-welcome-tagmode");
            winston.reply({ content: `> **✅ Taglı Alım Başarıyla Kapatıldı!**`, ephemeral: true,components:[winston_buttons]  })
        }
